...
public class CartItemController {
	...
	public CartItemPrice getCartByCusId(@PathVariable("customerId") Integer customerId)
		...
		List<CartItem> allcartitem = this.cartItemService.getProductByCusId(customerId);
		List<CartProduct> cartProduct = new ArrayList<CartProduct>();
		Double amount = 0.0;
		
		for(CartItem iterator : allcartitem){
			//CartProduct cartProductDto = null;
	...
	public CartProduct getProductFromCart(@PathVariable("customerId") Integer customerId,@PathVariable("productId")
	Integer productId) throws CartItemException, CustomerException, ProductException{
	...
	public String deleteProductFromCart(@PathVariable("customerId") Integer customerId,
			@PathVariable("productId") Integer productId) throws CustomerException, CartItemException, ProductException {
	...
	public String updateQuantityOfProduct(@PathVariable("customerId") Integer customerId,@PathVariable("productId") Integer productId,
			@PathVariable("quantity") Integer quantity)throws CustomerException, CartItemException, ProductException{
